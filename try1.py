...
for x in range(loops):
    ...
    I1_reduce = LK_Reduce_Iterative(root.filename, 1)

    root2 = Tk()
    root2.filename = filedialog.askopenfilename(initialdir="/", title="Select file",
                                                filetypes=(("png files", "*.png"), ("all files", "*.*")))

    I2_reduce = LK_Reduce_Iterative(root2.filename, 1)
    FinalReduceImage1 = Lucas_Kanade(root.filename, I1_reduce, root2.filename, I2_reduce, 3, "Reduce")
